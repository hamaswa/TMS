...
<main class="section"><div class="shell detail"><article class="card panel">@if($service->is_featured)<div class="featured">{{ __('storefront.tailoring.featured') }}</div>@endif<h1>{{ $service->name }}</h1><div class="meta">@if($service->price_from!==null)<span class="pill">{{ __('storefront.tailoring.starting_price') }}: {{ __('storefront.common.rs') }} {{ number_format($service->price_from,2) }} · {{ $service->price_unit }}</span>@endif @if($service->estimated_days)<span class="pill">{{ __('storefront.tailoring.estimated_time') }}: {{ $service->estimated_days }} {{ __('storefront.common.days') }}</span>@endif</div><p>{{ $service->description ?: __('storefront.tailoring.default_description') }}</p><div class="notice">{{ __('storefront.tailoring.price_notice') }}</div>@if($storefront->inquiries_enabled)<a class="btn" href="{{ route('storefront.tailoring.index',[$storefront,'service'=>$service->id]).'#inquiry' }}">{{ __('storefront.tailoring.inquire_service') }}</a>@elseif($storefront->public_phone)<a class="btn" href="tel:{{ preg_replace('/[^0-9+]/','',$storefront->public_phone) }}">{{ __('storefront.common.contact_shop') }}</a>@endif</article></div></main>
@endsection
