...
<head>
    ...
    @stack('head')
    <style>
        @font-face{font-family:"Noto Nastaliq Urdu";src:url('{{ asset('assets/fonts/noto-nastaliq-urdu/NotoNastaliqUrdu-VariableFont_wght.woff2') }}') format('woff2');font-display:swap;font-weight:100 900}
        :root{--ink:#17382e;--muted:#687d75;--primary:#126b4f;--primary-dark:#0b4937;--line:#dfe8e4;--paper:#fff;--bg:#f4f7f5;--danger:#923b3b}
        *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font-family:{{ app()->getLocale() === 'ur' ? '"Noto Nastaliq Urdu","Noto Sans Arabic",Tahoma,sans-serif' : 'Inter,Arial,sans-serif' }};line-height:{{ app()->getLocale() === 'ur' ? '2' : '1.7' }}}button,input,select,textarea{font-family:inherit}.shell{width:min(1180px,calc(100% - 28px));margin:auto}
