...
@endpush
@section('body')
<nav class="nav"><div class="shell"><a class="nav-brand" href="{{ route('storefront.show',$storefront) }}">{{ $storefront->display_name }}</a><div class="nav-actions">@include('storefront.public.partials.language-switch')@if($storefront->clothingOrderingEnabled())<a class="nav-link" href="{{ route('storefront.cart.show',$storefront) }}">{{ __('storefront.clothing.cart') }}</a>@endif<a class="nav-link" href="{{ route('storefront.clothing.index',$storefront) }}">{{ __('storefront.clothing.all_clothing') }}</a></div></div></nav>
...
<section>@if($listing->is_featured)<div class="featured">{{ __('storefront.clothing.featured') }}</div>@endif<h1>{{ $listing->display_name }}</h1><div class="muted">{{ $listing->cloth->brand->name ?? __('storefront.clothing.no_brand') }} · {{ $listing->cloth->type->name ?? __('storefront.clothing.fabric') }}</div><div class="price">{!! \App\Support\PakistanCurrency::html($listing->cloth->sale_price ?: $listing->cloth->price) !!} {{ __('storefront.clothing.per_metre') }}</div>@if($listing->description)<p>{{ $listing->description }}</p>@endif
...
@if($storefront->clothingOrderingEnabled() && $listing->acceptsOnlineOrders() && $totalAvailable>0)<div class="card" style="margin-top:20px">@if($errors->any())<div class="errors">{{ $errors->first() }}</div>@endif<form method="POST" action="{{ route('storefront.cart.store',[$storefront,$listing]) }}">@csrf<div class="cart-grid"><select name="cloth_color_id" class="control" required><option value="">{{ __('storefront.clothing.select_color') }}</option>@foreach($listing->cloth->colors as $color)@php($available=$color->reservableLength())<option value="{{ $color->id }}" @disabled($available<=0)>{{ $color->color }} — {{ number_format($available,2) }}m</option>@endforeach</select><input type="number" name="quantity" min="{{ $listing->minimumOrderQuantity() }}" max="{{ $listing->maximumOrderQuantity() }}" step="{{ $listing->orderIncrement() }}" value="{{ $listing->minimumOrderQuantity() }}" class="control" aria-label="{{ __('storefront.clothing.quantity_label') }}" required><button class="btn">{{ __('storefront.clothing.reserve') }}</button></div></form><small>{{ __('storefront.clothing.order_limits',['min'=>number_format($listing->minimumOrderQuantity(),2),'max'=>number_format($listing->maximumOrderQuantity(),2),'step'=>number_format($listing->orderIncrement(),2)]) }}</small><br><small>{{ __('storefront.clothing.reservation_note') }}</small></div>
