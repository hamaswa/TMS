...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}body{margin:0;background:#f4f7f5;color:#17382e;font-family:Nastaliq,Tahoma,sans-serif}.shell{width:min(1080px,calc(100% - 28px));margin:auto}.nav{background:#fff;border-bottom:1px solid #dde7e2}.nav .shell{min-height:70px;display:flex;align-items:center;justify-content:space-between}.nav a{color:#145d45;text-decoration:none;border:1px solid #bdd2c9;border-radius:10px;padding:7px 13px}
        ...
        @media(max-width:760px){.product{grid-template-columns:1fr;padding:28px 0}.photo:first-child{min-height:300px}.nav .shell{min-height:62px}}
    </style>
</head>
<body>
<nav class="nav"><div class="shell"><strong>{{ $storefront->display_name }}</strong><a href="{{ route('storefront.clothing.index',$storefront) }}">تمام کپڑے</a></div></nav>
<main class="shell product">
    ...
    <section class="details">
        ...
        <div class="colors">@foreach($listing->cloth->colors as $color)<div class="color"><strong>{{ $color->color }}</strong><span class="{{ (float)$color->length>0?'available':'out' }}">{{ (float)$color->length>0 ? number_format($color->length,2).' میٹر' : 'اسٹاک ختم' }}</span></div>@endforeach</div>
        <div class="notice">آن لائن آرڈر ابھی فعال نہیں۔ قیمت اور دستیابی تازہ انوینٹری سے دکھائی جا رہی ہے۔</div>
