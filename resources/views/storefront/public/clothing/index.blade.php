...
<article class="card product-card"><div class="photo">@if($image)<img src="{{ $image->image_url }}" alt="{{ $listing->display_name }}">@else<span>🧵</span>@endif</div><div class="product-body">@if($listing->is_featured)<div class="featured">{{ __('storefront.clothing.featured') }}</div>@endif<h2>{{ $listing->display_name }}</h2><div class="muted">{{ $listing->cloth->brand->name ?? __('storefront.clothing.no_brand') }} · {{ $listing->cloth->type->name ?? __('storefront.clothing.fabric') }}</div><div class="price">{!! \App\Support\PakistanCurrency::html($listing->cloth->sale_price ?: $listing->cloth->price) !!} {{ __('storefront.clothing.per_metre') }}</div><div>@if(!$listing->is_available){{ __('storefront.clothing.temporarily_unavailable') }}@elseif($available>0){{ __('storefront.clothing.available',['amount'=>number_format($available,2)]) }}@elseif($listing->preorder_enabled){{ __('storefront.clothing.preorder_available') }}@else{{ __('storefront.clothing.out_of_stock') }}@endif</div>@if(!$listing->online_order_enabled)<div class="muted">{{ __('storefront.clothing.catalogue_badge') }}</div>@endif<div class="colors">@foreach($listing->cloth->colors as $color)<span class="pill">{{ $color->color }} — {{ number_format($color->reservableLength(),2) }}m</span>@endforeach</div><a class="btn" href="{{ route('storefront.clothing.show',[$storefront,$listing]) }}">{{ __('storefront.clothing.view_details') }}</a></div></article>
