...
@php
    ...
    $recordLabels = ['advance' => 'ہفتہ وار ایڈوانس', 'main_advance' => 'مرکزی ایڈوانس', 'salary' => 'اجرت کی ادائیگی', 'chai' => 'چائے / خرچ'];
@endphp
...
<style>
    ...
    .tailor-overview{display:grid;grid-template-columns:repeat(2,minmax(220px,.7fr)) minmax(320px,1.6fr);gap:14px;margin-bottom:20px}.tailor-balance-card{display:flex;align-items:center;gap:14px;min-height:112px;padding:18px;border:1px solid var(--tr-line);border-radius:15px;background:#fff;box-shadow:0 6px 22px rgba(21,47,81,.05)}.tailor-balance-icon{display:grid;place-items:center;flex:0 0 48px;width:48px;height:48px;border-radius:50%;font-size:19px}.tailor-balance-card.is-advance .tailor-balance-icon{color:#d17d08;background:#fff3dd}.tailor-balance-card.is-security .tailor-balance-icon{color:#138355;background:#e7f7ef}.tailor-balance-card small{display:block;color:var(--tr-muted)}.tailor-balance-card strong{direction:ltr;display:block;margin-top:5px;color:var(--tr-navy);font-size:1.25rem;font-weight:800;text-align:right}
    .tailor-period{display:flex;align-items:center;justify-content:space-between;gap:20px;padding:18px 20px;border:1px solid #dce7f5;border-radius:15px;background:linear-gradient(135deg,#f5f9ff,#fff)}.tailor-period-copy{display:flex;align-items:center;gap:12px}.tailor-period-copy>i{display:grid;place-items:center;width:42px;height:42px;border-radius:12px;color:var(--tr-blue);background:#e7f1ff;font-size:18px}.tailor-period-copy small{display:block;color:var(--tr-muted)}.tailor-period-copy strong{display:block;margin-top:4px;color:var(--tr-navy);font-size:1rem}.tailor-filter{display:flex;align-items:end;gap:8px}.tailor-filter label{display:block;margin-bottom:6px;color:#52657e;font-size:.75rem;font-weight:800}.tailor-filter select{min-width:145px;height:41px;padding:7px 10px;border:1px solid #d2ddea;border-radius:9px;background:#fff}.tailor-filter button{height:41px;border:0;border-radius:9px;color:#fff;background:var(--tr-blue);font-weight:800;padding:0 14px}
    ...
    .tailor-record-badge{display:inline-block;padding:5px 9px;border-radius:999px;font-size:.72rem;font-weight:800}.tailor-record-badge.advance{color:#a66a0a;background:#fff3db}.tailor-record-badge.salary{color:#15784c;background:#e8f8f0}.tailor-record-badge.chai{color:#7650b9;background:#f1ebff}
    .tailor-settlement{padding:8px 18px;background:#fbfdff}.tailor-settlement-row{display:flex;align-items:center;justify-content:space-between;gap:15px;padding:12px 0;border-bottom:1px solid #e9eef5;color:#566980}.tailor-settlement-row:last-child{border-bottom:0}.tailor-settlement-row strong{direction:ltr;color:var(--tr-navy);font-size:.95rem}.tailor-settlement-row.is-due{margin:6px -8px;padding:13px 10px;border:0;border-radius:10px;color:#0f5fc7;background:#eaf3ff;font-weight:800}.tailor-settlement-row.is-due strong{color:#0f5fc7;font-size:1.08rem}.tailor-record-list{border-top:1px solid var(--tr-line)}.tailor-record-list-title{display:flex;align-items:center;justify-content:space-between;padding:13px 18px;color:var(--tr-navy);font-weight:800}.tailor-record-item{display:grid;grid-template-columns:1fr auto;gap:5px 12px;padding:12px 18px;border-top:1px solid #edf1f6}.tailor-record-item:first-of-type{border-top:0}.tailor-record-item .tailor-money{grid-row:1 / span 2;grid-column:2;align-self:center}.tailor-record-empty{padding:24px 18px;color:#8593a6;text-align:center}.tailor-record-empty i{display:block;margin-bottom:8px;color:#b4c0cf;font-size:1.5rem}
    ...
    @media(max-width:1150px){.tailor-overview{grid-template-columns:repeat(2,1fr)}.tailor-period{grid-column:1 / -1}.tailor-report-grid{grid-template-columns:1fr}}
    @media(max-width:767px){.tailor-report-page{padding-top:18px}.tailor-report-shell{width:min(100% - 20px,1700px)}.tailor-report-head{align-items:flex-start;flex-direction:column}.tailor-head-actions{width:100%}.tailor-btn{flex:1}.tailor-period{align-items:stretch;flex-direction:column}.tailor-filter{width:100%}.tailor-filter>div{flex:1}.tailor-filter select{width:100%;min-width:0}.tailor-balance-card{min-height:96px;padding:14px}.tailor-panel-head{align-items:flex-start;flex-direction:column}}
    @media(max-width:480px){.tailor-overview{grid-template-columns:1fr}.tailor-period{grid-column:auto}.tailor-filter{align-items:stretch;flex-direction:column}}
</style>
...
<section class="main-content tailor-report-page">
    <div class="tailor-report-shell">
        ...
        <div class="tailor-overview">
            <article class="tailor-balance-card is-advance"><span class="tailor-balance-icon"><i class="fas fa-wallet"></i></span><div><small>مرکزی قابلِ وصول ایڈوانس</small><strong>Rs. {{ number_format((float)$tailor->advance,2) }}</strong></div></article>
            <article class="tailor-balance-card is-security"><span class="tailor-balance-icon"><i class="fas fa-shield-alt"></i></span><div><small>دکان کے پاس سیکیورٹی ڈپازٹ</small><strong>Rs. {{ number_format((float)$tailor->security_deposit,2) }}</strong></div></article>
            <section class="tailor-period">
                <div class="tailor-period-copy"><i class="far fa-calendar-alt"></i><div><small>رپورٹ کی تاریخ</small><strong dir="ltr">{{ $periodRange }}</strong></div></div>
                <form method="GET" action="{{ route('admin.tailor-report',$tailor) }}" class="tailor-filter"><div><label for="filterType">مدت منتخب کریں</label><select id="filterType" name="filterType"><option value="weekly" @selected($filterType==='weekly')>موجودہ ہفتہ</option><option value="monthly" @selected($filterType==='monthly')>موجودہ مہینہ</option></select></div><button type="submit"><i class="fas fa-filter ml-1"></i> دکھائیں</button></form>
            </section>
        </div>
        ...
        <div class="tailor-report-grid">
            ...
            <aside class="tailor-panel">
                <div class="tailor-panel-head"><div><h2>حساب کا خلاصہ</h2><p>{{ $periodLabel }} کا سادہ مالی حساب</p></div></div>
                <div class="tailor-settlement">
                    <div class="tailor-settlement-row"><span>اجرت اور دیگر ادائیگیاں</span><strong>Rs. {{ number_format($paidAmount,2) }}</strong></div>
                    <div class="tailor-settlement-row"><span>ہفتہ وار ایڈوانس</span><strong>Rs. {{ number_format($periodAdvance,2) }}</strong></div>
                    @if($advanceCoveredFromMain > 0)<div class="tailor-settlement-row"><span>مرکزی ایڈوانس سے کٹوتی</span><strong>Rs. {{ number_format($advanceCoveredFromMain,2) }}</strong></div>@endif
                    @if($weeklyAdvanceFromPayments > 0)<div class="tailor-settlement-row"><span>ادائیگیوں سے منہا ایڈوانس</span><strong>- Rs. {{ number_format($weeklyAdvanceFromPayments,2) }}</strong></div>@endif
                    <div class="tailor-settlement-row is-due"><span>حتمی ہفتہ وار رقم</span><strong>Rs. {{ number_format($weeklySettlementTotal,2) }}</strong></div>
                </div>
                <div class="tailor-record-list">
                    <div class="tailor-record-list-title"><span>حالیہ اندراجات</span><span class="tailor-count">{{ $tailor_records->count() }}</span></div>
                    @forelse($tailor_records as $record)
                        <div class="tailor-record-item"><span class="tailor-record-badge {{ $record->comment }}">{{ $recordLabels[$record->comment] ?? $record->comment }}</span><span class="tailor-money">Rs. {{ number_format((float)$record->amount,2) }}</span><small class="tailor-secondary">{{ $record->created_at?->format('d-m-Y') }}</small></div>
                    @empty
                        <div class="tailor-record-empty"><i class="fas fa-receipt"></i>اس مدت میں کوئی ادائیگی درج نہیں۔</div>
                    @endforelse
                </div>
                ...
                @if($weeklyAdvanceFromPayments > 0)
                    <div class="tailor-advance-note"><i class="fas fa-info-circle ml-1"></i> باقی ہفتہ وار ایڈوانس <strong>Rs. {{ number_format($weeklyAdvanceFromPayments,2) }}</strong> اجرت اور دیگر ادائیگیوں سے منہا کیا گیا ہے۔</div>
                @endif
                @if($filterType==='weekly' && $weeklyAdvanceFromPayments>0 && (float)$tailor->advance>0)<div class="tailor-advance-note"><button type="button" class="tailor-btn" data-toggle="modal" data-target="#cutAdvanceModal"><i class="fas fa-minus-circle"></i> ہفتہ وار ایڈوانس کو مرکزی ایڈوانس سے کاٹیں</button></div>@endif
            </aside>
        </div>

        <div class="modal fade" id="addRecordModal" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><form method="post" action="{{ route('admin.tailor.addRecord',$tailor) }}">@csrf<div class="modal-header"><h5 class="modal-title"><i class="fas fa-money-check-alt text-success ml-2"></i> درزی کا مالی ریکارڈ</h5><button type="button" class="close mr-auto ml-0" data-dismiss="modal"><span>&times;</span></button></div><div class="modal-body"><div class="form-group"><label for="recordComment">لین دین کی قسم</label><select id="recordComment" name="comment" class="form-control" required><option value="salary">اجرت کی ادائیگی</option><option value="advance">ہفتہ وار ایڈوانس</option><option value="chai">چائے / دیگر خرچ</option></select><small class="form-text text-muted">ہفتہ وار ایڈوانس الگ حساب ہے؛ یہ مرکزی ایڈوانس میں شامل نہیں ہوگا۔</small></div><div class="form-group mb-0"><label for="recordAmount">رقم</label><div class="input-group" dir="ltr"><div class="input-group-prepend"><span class="input-group-text">Rs.</span></div><input id="recordAmount" type="number" min="0.01" step="0.01" name="amount" class="form-control" required></div></div></div><div class="modal-footer"><button type="submit" class="btn btn-success">محفوظ کریں</button><button type="button" class="btn btn-light" data-dismiss="modal">منسوخ کریں</button></div></form></div></div></div>

        <div class="modal fade" id="cutAdvanceModal" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><form method="post" action="{{ route('admin.tailor.cutAdvanceRecord',$tailor) }}">@csrf<input type="hidden" name="total" value="{{ $paidAmount }}"><div class="modal-header"><h5 class="modal-title">مرکزی ایڈوانس سے کٹوتی</h5><button type="button" class="close mr-auto ml-0" data-dismiss="modal"><span>&times;</span></button></div><div class="modal-body"><div class="alert alert-info">مرکزی ایڈوانس: <strong>Rs. {{ number_format((float)$tailor->advance,2) }}</strong><br>باقی ہفتہ وار ایڈوانس: <strong>Rs. {{ number_format($weeklyAdvanceFromPayments,2) }}</strong></div><div class="form-group mb-0"><label for="advanceCutAmount">کٹوتی کی رقم</label><input id="advanceCutAmount" type="number" min="0.01" max="{{ min((float)$tailor->advance,$weeklyAdvanceFromPayments) }}" step="0.01" name="amount" class="form-control" required></div></div><div class="modal-footer"><button type="submit" class="btn btn-primary">کٹوتی محفوظ کریں</button><button type="button" class="btn btn-light" data-dismiss="modal">منسوخ کریں</button></div></form></div></div></div>
    </div>
</section>
@endsection
