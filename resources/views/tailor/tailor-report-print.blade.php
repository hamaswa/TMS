    ...
    <style>
        ...
        * {
        ...
        .screen-actions button {
            ...
            color: #243b53;
            background: #fff;
            font: 700 14px 'Noto Nastaliq Urdu', Tahoma, sans-serif;
            cursor: pointer
        }

        .screen-actions .print {
            color: #fff;
            border-color: #1769e0;
            background: #1769e0
        }
        ...
        .receipt {
            ...
            padding: 3mm 4mm;
            background: #fff;
            box-shadow: 0 12px 36px rgba(18, 42, 70, .16);
            font-family: 'Noto Nastaliq Urdu', Tahoma, Arial, sans-serif;
            font-weight: 900
        }
        ...
        .work-head {
            display: grid;
            grid-template-columns: .9fr .8fr .65fr 1.15fr;
            gap: 1.5mm;
            padding: 1.5mm 1mm;
        ...
        .work-main .rate-breakdown,
        .work-main .work-total {
            direction: ltr;
            text-align: left;
            white-space: nowrap;
        ...
        .work-main .serial-list {
            direction: ltr;
            text-align: left;
            overflow-wrap: anywhere;
            font: 900 8px/1.7 Tahoma, Arial, sans-serif
        }
        ...
        .money-line.deduction strong:before {
            content: '− '
        }
...
<body>
    @php
        ...
        $workSummary = $tailor_report
            ...
            ->map(function ($orders, $sewingName) {
                ...
                $rateBreakdown = $rateGroups
                    ->map(function ($rateOrders, $rate) {
    ...
    <main class="receipt" aria-label="درزی کا {{ $periodLabel }} حساب">
        <header class="receipt-header">
            @if ($setting?->logo_url)
                <img class="receipt-logo" src="{{ $setting->logo_url }}" alt="{{ $shopName }}">
            @endif
        ...
        <div class="section-title"><span>سلائی کا ریکارڈ</span><span>{{ $tailor_report->count() }} آرڈرز ·
                {{ $totalSuits }} سوٹ</span></div>
        @if ($workSummary->isNotEmpty())
            <div class="work-head"><span>سلائی کی قسم</span><span>سوٹ × اجرت</span><span>کل اجرت</span><span>سیریل
                    نمبر</span></div>
            @foreach ($workSummary as $work)
                <article class="work-item">
                    <div class="work-main">
                        ...
                        <span
                            class="serial-list">{{ $work['serials']->isNotEmpty() ? $work['serials']->implode(', ') : '—' }}</span>
                    </div>
                </article>
            @endforeach
        @else
        ...
        <section class="money-lines">
            ...
            <div class="money-line"><span>دیگر ادائیگی</span><strong>Rs.
                    {{ number_format($otherExpenseAmount, 2) }}</strong></div>
            <div class="money-line"><span>{{ $periodLabel }} ایڈوانس</span><strong>Rs.
                    {{ number_format($weeklyAdvance, 2) }}</strong></div>
            @if ($advanceCoveredFromMain > 0)
                <div class="money-line"><span>مرکزی ایڈوانس سے کٹوتی</span><strong>Rs.
                        {{ number_format($advanceCoveredFromMain, 2) }}</strong></div>
            @endif
            @if ($advanceToDeductFromWeeklyPayment > 0)
                <div class="money-line"><span>ادائیگیوں سے منہا ایڈوانس</span><strong>- Rs.
                        {{ number_format($advanceToDeductFromWeeklyPayment, 2) }}</strong></div>
            @endif
            <div class="money-line final"><span>حتمی {{ $periodLabel }} رقم</span><strong>Rs.
                    {{ number_format($weeklySettlementTotal, 2) }}</strong></div>
        </section>
        @if ($advanceCoveredFromMain > 0)
            <div class="advance-note"><strong>Rs. {{ number_format($advanceCoveredFromMain, 2) }}</strong> مرکزی ایڈوانس
                سے کاٹا جا چکا ہے۔</div>
        @endif

        <footer class="receipt-footer">
            @if ($setting?->address)
                <div>{{ strip_tags($setting->address) }}</div>
            @endif
            @if ($setting?->contact_no)
                <div class="contact">{{ $setting->contact_no }}</div>
            @endif
