...
@section('content')
    ...
    <style>
        ...
        .td-notice.is-success {
            color: #146c43;
            background: #eaf8f1;
            border-color: #ccebdc;
        }

        .td-notice.is-warning {
            color: #805800;
            background: #fff8e5;
            border-color: #f4dfaa;
        }
        ...
        .td-action.is-primary {
            color: #fff;
            border-color: var(--td-blue);
            background: var(--td-blue);
        }

        .td-action.is-success {
            color: #087d50;
            border-color: #bfe6d3;
            background: #effbf5;
        }
        ...
        .td-modal .modal-content {
            ...
            border: 0;
            border-radius: 15px;
            box-shadow: 0 22px 65px rgba(15, 38, 70, .2);
        }
        ...
        @media (max-width: 767px) {
            ...
            .td-table tbody td:first-child::before {
                display: none;
            }
        ...
        <div class="td-shell">
            <div class="td-page-head">
                <div class="td-title">
                    ...
                    <div>
                        <h1>درزی اور کاریگر</h1>
                        <p>درزیوں کی اجرت، ایڈوانس، سیکیورٹی، نرخ اور آرڈرز ایک جگہ منظم کریں۔</p>
                    </div>
                </div>
            ...
            @endforeach
            @if ($errors->any())
                <div class="td-notice is-danger" role="alert"><i class="fas fa-exclamation-circle"></i>
                    <div>
            ...
            @if ($tailorLimit !== null)
                <div class="td-limit {{ $tailorLimitReached ? 'is-reached' : '' }}">
                    <i class="fas fa-layer-group ml-1"></i> پلان حد: <strong>{{ $Tailors->count() }} /
                        {{ $tailorLimit }}</strong> درزی
                    @if ($tailorLimitReached)
                        — مزید درزی کے لیے پلان اپ گریڈ کریں۔
                    @endif
                </div>
            @endif
            ...
            <div class="td-panel">
                <div class="td-panel__head">
                    <div class="td-panel__title">
                        <h2>درزیوں کی فہرست</h2>
                        <p>ہر درزی کا موجودہ حساب دیکھیں یا متعلقہ کام پر براہِ راست جائیں۔</p>
                    </div>
                    <label class="td-search" for="tailorDirectorySearch"><i class="fas fa-search"></i><input
                            id="tailorDirectorySearch" type="search" autocomplete="off"
                            placeholder="نام یا فون نمبر سے تلاش کریں"></label>
                </div>
                    ...
                    <table class="td-table" id="tailorDirectoryTable">
                        ...
                        <tbody>
                            @forelse($Tailors as $tailor)
                                ...
                                <tr data-tailor-row>
                                    <td data-label="درزی">
                                        <div class="td-person"><span class="td-avatar">{{ $initial }}</span>
                                            <div><strong>{{ $tailor->name }}</strong><small>{{ $tailor->tailorsalary->count() }}
                                                    سلائی نرخ محفوظ</small></div>
                                        </div>
                                    </td>
                                    <td data-label="رابطہ نمبر"><span
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addRecordModalLabel_{{ $tailor->id }}"><i
                                class="fas fa-hand-holding-usd text-success ml-2"></i>{{ $tailor->name }} کو ایڈوانس دیں
                        </h5><button type="button" class="close" data-dismiss="modal"
                            aria-label="بند کریں">&times;</button>
                    </div>
                    <form method="post" action="{{ route('admin.tailor.addAdvanceRecord', $tailor->id) }}">@csrf
                        <div class="modal-body">
                            <div class="td-help mb-3">یہ رقم دکان درزی کو دے رہی ہے۔ اسے قابلِ وصول ایڈوانس میں شامل کر کے
                                آئندہ اجرت سے واپس لیا جا سکتا ہے۔</div>
                            <div class="form-group mb-0"><label for="advance_amount_{{ $tailor->id }}">ایڈوانس رقم
                                    (روپے)</label><input id="advance_amount_{{ $tailor->id }}" type="number"
                                    min="0.01" step="0.01" name="amount" class="form-control"
                                    placeholder="مثلاً 500" required></div>
                        </div>
                        <div class="modal-footer"><button type="button" class="td-btn" data-dismiss="modal">منسوخ
                                کریں</button><button type="submit" class="td-btn is-primary"><i class="fas fa-save"></i>
                                ایڈوانس محفوظ کریں</button></div>
                    </form>
                </div>
            </div>
        </div>

        <div class="modal fade td-modal" id="securityDepositModal_{{ $tailor->id }}" tabindex="-1" role="dialog"
            aria-labelledby="securityDepositModalLabel_{{ $tailor->id }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="securityDepositModalLabel_{{ $tailor->id }}"><i
                                class="fas fa-shield-alt text-primary ml-2"></i>{{ $tailor->name }} کی سیکیورٹی ڈپازٹ</h5>
                        <button type="button" class="close" data-dismiss="modal"
                            aria-label="بند کریں">&times;</button>
                    </div>
                    <form method="post" action="{{ route('admin.tailor.securityDeposit', $tailor->id) }}">@csrf
                        <div class="modal-body">
                            <div class="td-help mb-3">سیکیورٹی ڈپازٹ درزی سے وصول کی گئی امانتی رقم ہے۔ درزی کو دیا گیا
                                ایڈوانس الگ حساب میں رہتا ہے۔</div>
                            <div class="form-group"><label for="security_type_{{ $tailor->id }}">لین دین کی
                                    قسم</label><select id="security_type_{{ $tailor->id }}" name="transaction_type"
                                    class="form-control" required>
                                    <option value="received">درزی سے مزید رقم وصول کی</option>
                                    <option value="refunded">درزی کو سیکیورٹی واپس کی</option>
                                </select></div>
                            <div class="form-group"><label for="security_amount_{{ $tailor->id }}">رقم
                                    (روپے)</label><input id="security_amount_{{ $tailor->id }}" type="number"
                                    min="0.01" step="0.01" name="amount" class="form-control"
                                    placeholder="مثلاً 1000" required></div>
                            <div class="form-group mb-0"><label for="security_note_{{ $tailor->id }}">نوٹ</label><input
                                    id="security_note_{{ $tailor->id }}" type="text" maxlength="500"
                                    name="note" class="form-control"
                                    placeholder="مثلاً رسید نمبر یا واپسی کی وجہ"></div>
                        </div>
                        <div class="modal-footer"><button type="button" class="td-btn" data-dismiss="modal">منسوخ
                                کریں</button><button type="submit" class="td-btn is-primary"><i class="fas fa-save"></i>
                                ریکارڈ محفوظ کریں</button></div>
                    </form>
                </div>
            </div>
        </div>
