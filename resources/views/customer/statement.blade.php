...
<section class="main-content" dir="rtl">
    <div class="container-fluid px-3 px-md-4 py-4">
        ...
        <div class="row">
            ...
            @endif
            @if($canViewShop)
                <div class="col-lg-6 mb-4"><div class="card h-100"><div class="card-header text-right"><strong>حالیہ دکان فروخت</strong></div><div class="table-responsive"><table class="table mb-0 text-right"><thead><tr><th>فروخت</th><th>تاریخ</th><th>اشیاء</th></tr></thead><tbody>@forelse($sales as $sale)<tr><td>#{{ $sale->id }}</td><td>{{ $sale->created_at?->format('d-m-Y') }}</td><td>{{ $sale->items_count }}</td></tr>@empty<tr><td colspan="3" class="text-center text-muted">کوئی دکان فروخت نہیں۔</td></tr>@endforelse</tbody></table></div></div></div>
            @endif
        </div>
    </div>
</section>
