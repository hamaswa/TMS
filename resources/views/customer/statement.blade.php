            ...
            <div class="card workspace-card mb-4"><div class="card-body p-4 p-lg-5">
                ...
                <div class="family-profile-grid">
                    @foreach($measurementProfiles as $profile)
                        <a class="family-profile-card {{ $measurementProfile->id === $profile->id ? 'active' : '' }}" href="{{ route('admin.customers.statement', ['id' => $customer->id, 'tab' => 'measurements', 'profile' => $profile->id]) }}">
                            <span class="family-profile-avatar">{{ \Illuminate\Support\Str::substr($profile->name, 0, 1) }}</span>
                            <span class="family-profile-copy"><strong>{{ $profile->name }}</strong><small>{{ $profile->id === $customer->id ? 'مرکزی گاہک' : 'خاندانی فرد' }} · {{ $profile->measurementTemplate?->name ?: 'تمام پیمائش' }}</small></span>
                            <span class="family-profile-count">{{ $profile->measurement_values_count }} اضافی</span>
                        </a>
                    @endforeach
                </div>
            </div></div>
...
<style>
    ...
    .family-profile-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:.8rem}.family-profile-card{display:flex;align-items:center;gap:.75rem;padding:1rem;border:1px solid #dfe7f0;border-radius:14px;background:#fbfdff;color:#29435f;text-decoration:none!important}.family-profile-card:hover{border-color:#8dbbe0;color:#1769aa}.family-profile-card.active{border-color:#1769aa;background:#eef7fd;box-shadow:0 0 0 2px rgba(23,105,170,.08)}.family-profile-avatar{display:flex;align-items:center;justify-content:center;flex:0 0 42px;height:42px;border-radius:12px;background:#1769aa;color:#fff;font-size:1.15rem;font-weight:800}.family-profile-copy{min-width:0;flex:1}.family-profile-copy strong,.family-profile-copy small{display:block}.family-profile-copy small{margin-top:.2rem;color:#718096;font-size:.76rem}.family-profile-count{white-space:nowrap;color:#52667a;font-size:.72rem;font-weight:700}
