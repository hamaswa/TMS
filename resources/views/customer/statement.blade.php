...
>>>>>>> 6271f2ce5fa255c1c46d019433714674d44eca6a
        ...
        @if($activeTab === 'shop' && $canViewShop)
            <div class="card workspace-card"><div class="card-body p-0"><div class="section-heading p-4 mb-0"><div><h2 class="h5 font-weight-bold">کپڑے کی خریداری</h2><p>دکان سے خریدی گئی اشیاء، رقم اور بقایا</p></div><span class="badge badge-primary px-3 py-2">{{ $sales->count() }} رسیدیں</span></div><div class="table-responsive"><table class="table table-hover mb-0 text-right"><thead><tr><th>رسید</th><th>تاریخ</th><th>تفصیل</th><th>اشیاء</th><th>کل رقم</th><th>وصول شدہ</th><th>بقایا</th><th></th></tr></thead><tbody>
                @forelse($sales as $sale)<tr class="{{ $sale->status === 'cancelled' ? 'text-muted' : '' }}"><td>#{{ $sale->id }} @if($sale->status === 'cancelled')<span class="badge badge-danger">منسوخ</span>@endif</td><td>{{ $sale->created_at?->format('d-m-Y') }}</td><td>{{ $sale->summary ?: 'کپڑے کی فروخت' }}@if($sale->cancellation_reason)<br><small>وجہ: {{ $sale->cancellation_reason }}</small>@endif</td><td>{{ $sale->items_count }}</td><td>Rs {{ number_format($sale->amount, 2) }}</td><td>Rs {{ number_format($sale->status === 'cancelled' ? 0 : $sale->received, 2) }}</td><td>Rs {{ number_format($sale->status === 'cancelled' ? 0 : $sale->balance, 2) }}</td><td>@if($sale->source === 'stock')<a class="btn btn-sm btn-outline-primary" href="{{ route('admin.printStock', ['id' => $sale->id, 'customerId' => $customer->id]) }}">رسید</a>@elseif($sale->source === 'legacy')<a class="btn btn-sm btn-outline-primary" href="{{ route('admin.sale.show', $sale->id) }}">تفصیل</a>@else<span class="text-muted">—</span>@endif</td></tr>
        ...
        @if($activeTab === 'measurements' && $canManageMeasurements)
            <div class="card workspace-card mb-4"><div class="card-body p-4 p-lg-5">
                <div class="section-heading"><div><h2 class="h4 font-weight-bold">خاندان کے محفوظ ناپ</h2><p>ایک فون نمبر اور مشترکہ کھاتے کے تحت ہر فرد کا نام اور پیمائش الگ رکھیں۔</p></div><a class="btn btn-success" href="{{ route('admin.Customers.create', ['parent' => $customer->id]) }}"><i class="fas fa-user-plus ml-1"></i> نیا خاندانی ناپ شامل کریں</a></div>
                <form class="family-profile-selector" method="GET" action="{{ route('admin.customers.statement', $customer->id) }}">
                    <input type="hidden" name="tab" value="measurements">
                    <div class="family-profile-field">
                        <label for="family-profile-select">جس فرد کے ناپ دیکھنے ہیں اسے منتخب کریں</label>
                        <select id="family-profile-select" name="profile" class="form-control" onchange="this.form.submit()">
                            @foreach($measurementProfiles as $profile)
                                <option value="{{ $profile->id }}" @selected($measurementProfile->id === $profile->id)>
                                    {{ $profile->name }} — {{ $profile->id === $customer->id ? 'مرکزی گاہک' : 'خاندانی فرد' }} — {{ $profile->measurementTemplate?->name ?: 'تمام پیمائش' }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <button class="btn btn-primary family-profile-submit" type="submit"><i class="fas fa-ruler ml-1"></i> ناپ دکھائیں</button>
                    <div class="family-profile-current" aria-live="polite">
                        <span class="family-profile-avatar">{{ \Illuminate\Support\Str::substr($measurementProfile->name, 0, 1) }}</span>
                        <span><small>اس وقت منتخب</small><strong>{{ $measurementProfile->name }}</strong></span>
                        <span class="family-profile-count">{{ $measurementProfile->measurement_values_count }} اضافی پیمائش</span>
                    </div>
                </form>
            </div></div>
            <div class="card workspace-card"><div class="card-body p-4 p-lg-5"><div class="section-heading"><div><h2 class="h4 font-weight-bold">{{ $measurementProfile->name }} کی محفوظ شدہ پیمائش</h2><p>سسٹم اور کاروبار کے اپنے اضافی پیمائشی خانے @if($measurementProfile->measurementTemplate)· ٹیمپلیٹ: <strong>{{ $measurementProfile->measurementTemplate->name }}</strong>@endif</p></div><a class="btn btn-outline-primary" href="{{ route('admin.Customers.edit', $measurementProfile) }}"><i class="fas fa-edit ml-1"></i> پیمائش تبدیل کریں</a></div>
            ...
            <div class="card workspace-card mt-4"><div class="card-body p-4 p-lg-5"><div class="section-heading"><div><h2 class="h4 font-weight-bold">پیمائش کی تاریخ</h2><p>ہر حقیقی تبدیلی کا پرانا اور نیا ریکارڈ محفوظ رہتا ہے۔</p></div><span class="badge badge-secondary px-3 py-2">{{ $measurementHistories->count() }} ورژن</span></div>
                <div class="measurement-history-list">
                    @forelse($measurementHistories as $history)
                        <details class="measurement-history-item" @if($loop->first) open @endif><summary><div><strong>{{ $historySourceLabels[$history->source] ?? 'محفوظ پیمائش' }}</strong><small>{{ $history->created_at?->format('d-m-Y h:i A') }} · {{ $history->recorder?->name ?: 'سسٹم' }} @if($history->template)· {{ $history->template->name }}@endif</small></div><span>{{ $history->values->count() }} خانے <i class="fas fa-chevron-down mr-1"></i></span></summary><div class="measurement-grid history-values">@foreach($history->values as $value)<div class="measurement-item"><small>{{ $value->label }}</small><strong>{{ $value->value }} @if(!in_array($value->unit, [null, '', 'none'], true))<span>{{ $value->unit === 'inch' ? 'انچ' : ($value->unit === 'cm' ? 'سینٹی میٹر' : $value->unit) }}</span>@endif</strong></div>@endforeach</div></details>
                    @empty<div class="empty-state">اگلی پیمائش تبدیلی کے بعد تاریخ یہاں دکھائی جائے گی۔ موجودہ پیمائش محفوظ ہے۔</div>@endforelse
                </div>
            </div></div>
        @endif
...
<style>
    ...
    .workspace-card{border:0;border-radius:18px;box-shadow:0 9px 26px rgba(31,45,61,.07);overflow:hidden}.profile-stat{height:100%;border:0;border-radius:17px;background:#fff;padding:1.25rem;box-shadow:0 8px 22px rgba(31,45,61,.07);display:flex;flex-direction:column;position:relative}.profile-stat small{display:block;color:#718096;line-height:1.9}.profile-stat strong{display:block;font-size:1.65rem;line-height:1.4;margin:.45rem 0}.profile-stat>span:last-child,.profile-stat a{font-size:.82rem;line-height:1.9;color:#718096}.profile-stat .stat-icon{position:absolute;left:1.1rem;top:1.1rem;width:39px;height:39px;border-radius:11px;background:#edf5fb;color:#1769aa;display:flex;align-items:center;justify-content:center}.profile-stat.balance{border-right:4px solid #1769aa}.profile-stat.paid{border-right:4px solid #20a06b}
    .section-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;margin-bottom:1.35rem}.section-heading h2,.section-heading p{margin-bottom:0}.section-heading h2{line-height:1.65}.section-heading p{color:#718096;font-size:.9rem;line-height:1.95;margin-top:.3rem}.activity-row{display:flex;align-items:center;gap:.75rem;padding:.85rem 0;border-bottom:1px solid #edf2f7}.activity-row:last-child{border-bottom:0}.activity-row small{display:block;color:#718096;line-height:1.9;margin-top:.15rem}.activity-icon{width:40px;height:40px;border-radius:12px;display:flex;align-items:center;justify-content:center;flex:0 0 auto}.activity-icon.tailoring{background:#f0eafd;color:#7048b5}.activity-icon.shop{background:#e7f7ef;color:#198754}
    ...
    .measurement-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:.8rem}.measurement-item{border:1px solid #dfe7f0;border-radius:14px;padding:1rem;background:#fbfdff}.measurement-item.custom{border-color:#cfe7d9;background:#f5fbf8}.measurement-item small,.measurement-item strong{display:block}.measurement-item small{color:#718096;line-height:1.9;margin-bottom:.35rem}.measurement-item strong{font-size:1.2rem;line-height:1.55}.measurement-item strong span{font-size:.75rem;color:#718096}.grid-full{grid-column:1/-1}
    .family-profile-selector{display:grid;grid-template-columns:minmax(260px,1fr) auto minmax(230px,auto);align-items:end;gap:1rem;padding:1rem;border:1px solid #dfe7f0;border-radius:14px;background:#fbfdff}.family-profile-field label{display:block;margin-bottom:.45rem;color:#52667a;font-weight:700}.family-profile-field .form-control{min-height:52px;border-color:#cdd9e5;border-radius:10px;background:#fff}.family-profile-submit{min-height:52px;white-space:nowrap}.family-profile-current{display:flex;align-items:center;gap:.7rem;min-height:52px;padding:.45rem .7rem;border-right:1px solid #dfe7f0}.family-profile-current small,.family-profile-current strong{display:block;line-height:1.7}.family-profile-current small{color:#718096}.family-profile-avatar{display:flex;align-items:center;justify-content:center;flex:0 0 42px;height:42px;border-radius:12px;background:#1769aa;color:#fff;font-size:1.15rem;font-weight:800}.family-profile-count{margin-right:auto;white-space:nowrap;color:#52667a;font-size:.76rem;font-weight:700}
    .measurement-history-item{border:1px solid #dfe7f0;border-radius:14px;background:#fff;margin-bottom:.75rem;overflow:hidden}.measurement-history-item summary{display:flex;justify-content:space-between;align-items:center;gap:1rem;padding:1rem 1.1rem;cursor:pointer;list-style:none;background:#f8fafc}.measurement-history-item summary::-webkit-details-marker{display:none}.measurement-history-item summary small{display:block;color:#718096;margin-top:.2rem}.measurement-history-item summary>span{color:#52667a;font-size:.85rem;white-space:nowrap}.measurement-history-item[open] summary{border-bottom:1px solid #e4ebf1;background:#eef7fd}.history-values{padding:1rem}.history-values .measurement-item{background:#fff}
    ...
    @media(max-width:991px){.family-profile-selector{grid-template-columns:1fr auto}.family-profile-current{grid-column:1/-1;border-right:0;border-top:1px solid #dfe7f0;padding-top:.8rem}}
    @media(max-width:767px){.customer-hero{padding:1.3rem}.customer-avatar{width:50px;height:50px}.profile-details{grid-template-columns:1fr}.profile-details .wide{grid-column:auto}.section-heading{align-items:flex-start;flex-direction:column}.family-profile-selector{grid-template-columns:1fr}.family-profile-submit{width:100%}.family-profile-current{grid-column:auto}.family-profile-count{white-space:normal}}
</style>
@endsection
