...
<section class="main-content"><div class="container-fluid px-3 px-md-4 py-4">
    ...
    <div class="row">
        <div class="col-lg-8">
            ...
            <div class="card mb-4"><div class="card-header bg-white d-flex justify-content-between align-items-center"><h5 class="mb-0">Public storefront</h5>@if($business->storefront?->is_published)<span class="badge badge-success">Published</span>@else<span class="badge badge-secondary">Not published</span>@endif</div><div class="card-body">
                @if($business->storefront)
                    <dl class="row mb-0"><dt class="col-sm-4">Display name</dt><dd class="col-sm-8">{{ $business->storefront->display_name }}</dd><dt class="col-sm-4">Public URL</dt><dd class="col-sm-8"><a target="_blank" rel="noopener" href="{{ $business->storefront->is_published ? route('storefront.show',$business->storefront) : '#' }}">/shops/{{ $business->storefront->slug }}</a></dd><dt class="col-sm-4">Public modules</dt><dd class="col-sm-8">{{ collect([$business->storefront->show_clothing ? 'Clothing' : null,$business->storefront->show_tailoring ? 'Tailoring' : null])->filter()->join(', ') ?: 'None' }}</dd><dt class="col-sm-4">Published</dt><dd class="col-sm-8">{{ $business->storefront->published_at?->format('d M Y, h:i A') ?? 'No' }}</dd></dl>
                @else
